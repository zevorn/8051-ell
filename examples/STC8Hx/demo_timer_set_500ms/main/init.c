...
void STC8x_TIMER_Config(void)
{
	...
	TIMER_InitStruct.SysClkDiv = 0xf5;
    TIMER_InitStruct.TCycle = TIMER_TCY_12T;   //Instruction cycle
    TIMER_InitStruct.ClkOut = DISABLE;           //Programmable clock output
    TIMER_InitStruct.Time = 500000UL;    //Loading initial value
    TIMER_InitStruct.Run = ENABLE;              //Operation control bit
	
	if(TIMER2_Init(&TIMER_InitStruct) != FSC_SUCCESS)
	{
		printf("Timer2 init erro! \r\n");
	}
	else
	{
		printf("Timer2 init success! \r\n");
	}
